...
class Generator
{
     ...
     * @return bool
     */
    public function generate(ShopInfo $shopInfo, array $currencies, array $categories, array $offers, array $deliveries = [], array $pickups = [])
    {
        try {
            $this->addHeader();
     ...
     * @param Pickup $pickup
     */
    protected function addPickup(Pickup $pickup)
    {
        $this->writer->startElement('option');
        $this->writer->writeAttribute('cost', $pickup->getCost());
        $this->writer->writeAttribute('days', $pickup->getDays());
        if ($pickup->getOrderBefore() !== null) {
            $this->writer->writeAttribute('order-before', $pickup->getOrderBefore());
        }
        $this->writer->endElement();
    }
    ...
    protected function addOffer(OfferInterface $offer)
    {
        ...
        $this->addOfferPickupOptions($offer);
    ...
    /**
     * Adds <pickup-option> element. (See https://yandex.ru/support/partnermarket/elements/pickup-options.html)
     *
     * @param array $pickups
     */
    private function addPickups(array $pickups)
    {
     ...
     * @param OfferInterface $offer
     */
    private function addOfferPickupOptions(OfferInterface $offer)
    {
        $options = $offer->getPickupOptions();
        if (!empty($options)) {
            $this->addPickups($options);
        }
    }
